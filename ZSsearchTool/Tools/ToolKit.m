...
+ (NSString *)creatTimeFromDate:(NSNumber *)date
{
    ...
    NSString * string=[NSString stringWithFormat:@"%f",d];
    long long int date1 = (long long int)[string integerValue];
...
//    NSDictionary *dicPlist  =  [NSPropertyListSerialization propertyListFromData:plistData mutabilityOption:NSPropertyListImmutable format: & format errorDescription: & error];
    
    NSDictionary *dic = [NSPropertyListSerialization propertyListWithData:plistData options:NSPropertyListImmutable format:&format error:&error];
    
    return dic;
}

// 查找并截取字符串
+(NSString*) findSubString:(NSString*)sFull :(NSString*)sLeft :(NSString*)sRight
{
    NSString *sSubString = nil;
    
    if ((nil!=sFull) && sFull.length) {
        NSRange start = [sFull rangeOfString:sLeft];
        
        if ( start.location!=NSNotFound ) {
            NSString *sFindLeft = [sFull substringFromIndex:start.location+start.length];
            
            NSRange end = [sFindLeft rangeOfString:sRight];
            
            if ( end.location != NSNotFound ) {
                sSubString = [sFindLeft substringToIndex:end.location];
                //sSubString = [sFull substringWithRange:NSMakeRange(start.location+start.length, end.location-start.location-start.length)];
            }
        }
    }
